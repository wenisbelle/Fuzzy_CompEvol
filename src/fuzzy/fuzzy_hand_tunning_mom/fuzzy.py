...
class FuzzyEvaluator:
    def __init__(self, map_width: int, map_height: int, camera_angle:float, distance_between_cells: int =10):
        ...
        self.one_cell_priority = ctrl.Consequent(np.arange(0, 1.0, 0.01), 'one_cell_priority', defuzzify_method = 'mom')

        self.uncertainty['low'] = fuzz.trapmf(self.uncertainty.universe, [-1, 0, 2, 5])
        self.uncertainty['medium'] = fuzz.trimf(self.uncertainty.universe, [2, 5, 7])
        self.uncertainty['high'] = fuzz.trapmf(self.uncertainty.universe, [5, 7, 30, 31])
        
        self.distance['close'] = fuzz.trapmf(self.distance.universe, [-1, 0, 40, 80])
        self.distance['medium'] = fuzz.trimf(self.distance.universe, [40, 80, 120])
        self.distance['far'] = fuzz.trapmf(self.distance.universe, [80, 120, 150, 151])

        self.individual_cell_uncertainty['low'] = fuzz.trapmf(self.individual_cell_uncertainty.universe, [-0.1, 0.0, 0.25, 0.5])
        self.individual_cell_uncertainty['medium'] = fuzz.trimf(self.individual_cell_uncertainty.universe, [0.25, 0.5, 0.75])
        self.individual_cell_uncertainty['high'] = fuzz.trapmf(self.individual_cell_uncertainty.universe, [0.5, 0.75, 2.0, 2.1])

        self.one_cell_priority['very_low'] = fuzz.trimf(self.one_cell_priority.universe, [-0.1, 0.0, 0.25])
        self.one_cell_priority['low'] = fuzz.trimf(self.one_cell_priority.universe, [0.0, 0.25, 0.5])
        self.one_cell_priority['medium'] = fuzz.trimf(self.one_cell_priority.universe, [0.25, 0.5, 0.75])
        self.one_cell_priority['high'] = fuzz.trimf(self.one_cell_priority.universe, [0.5, 0.75, 1.0])
        self.one_cell_priority['very_high'] = fuzz.trimf(self.one_cell_priority.universe, [0.75, 1.0, 1.1])
        ...
        self.pair_priority = ctrl.Consequent(np.arange(0, 1.0, 0.01), 'pair_priority', defuzzify_method = 'mom')
        ...
        self.distance_between_targets['close'] = fuzz.trapmf(self.distance_between_targets.universe, [-1, 0, 40, 80])
        self.distance_between_targets['medium'] = fuzz.trimf(self.distance_between_targets.universe, [40, 80, 120])
        self.distance_between_targets['far'] = fuzz.trapmf(self.distance_between_targets.universe, [80, 120, 150, 151])
...
"""
def main():
    evaluator = FuzzyEvaluator(map_width=10, map_height=10, camera_angle=np.radians(30))
