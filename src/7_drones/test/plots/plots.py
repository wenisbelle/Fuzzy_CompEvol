        ...
        for metric_idx in range(3): # 0: uncertainty, 1: unvisited, 2: accomulated uncertainty
            ...
            for sim_data in all_simulations_data:
                if uav_id in sim_data and sim_data[uav_id][metric_idx].size > 0:
                    ...
                    # --- CRITICAL FIX 1: Sort timestamps chronologically ---
            ...
            for run_vals, run_times in all_runs_for_metric:
                ...
                # --- CRITICAL FIX 2: Handle edge padding safely ---
...
Fuzzy_first_individual = parse_and_average_logs_from_folder("/FuzzyGA/src/definitive_system/coordination_only/7_drones/test/logs/fuzzy_2000")

Analytical_individual = parse_and_average_logs_from_folder("/FuzzyGA/src/definitive_system/coordination_only/7_drones/test/logs/analytical_2000")

Greed = parse_and_average_logs_from_folder("/FuzzyGA/src/definitive_system/coordination_only/7_drones/greed/logs")

eGreed = parse_and_average_logs_from_folder("/FuzzyGA/src/definitive_system/coordination_only/7_drones/e_greed/logs")
