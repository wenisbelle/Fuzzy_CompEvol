...
class FitnessEvaluator:
    ...
    def choose_one_cell(self, map_data: np.array, drone_position: Tuple[float, float, float], map_center_offset: float) -> list:
        ...
        if random.random() <= 0.1:
            x_random = random.randint(0, self.map_width)
            y_random = random.randint(0, self.map_width)
            target_coords = [x_random, y_random]
        else:
    ...
    def choose_two_cells(self, map_data: np.array, first_drone_pos, second_drone_pos, map_center_offset) -> list:
        ...
        if random.random() <= 0.1:
            x_random = random.randint(0, self.map_width)
            y_random = random.randint(0, self.map_width)
            first_target_coords = [x_random, y_random]
        else:
        ...
        if random.random() <= 0.1:
            x_random = random.randint(0, self.map_width)
            y_random = random.randint(0, self.map_width)
            second_drone_value = 1.0
            second_target_coords = [x_random, y_random]
        else:
